...
main_url = "http://tech.163.com/special/gd2016_"
#翻页次数
i = 1
count = 20
timeout = 5
...
while i <= count:
    try:
        ...
        logger.info(soup)
        a_lists = soup.select("ul#news-flow-content li div h3 a")

        a_lists_2 = soup.select("div.mod_cmr_list ul li a")
        logger.info(a_lists)
        for a in a_lists:
            detail_urls.append(a.get("href"))
...
send_dingding(["资讯--科技板块去重后一共推送 :%s" %crawlcount])