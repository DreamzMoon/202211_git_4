    ...
    try:
        ...
        if mode == 'create':
            ...
            buy_sql = '''
                ...
                where del_flag=0 and type = 1 and `status`=1 and (phone is not null or phone != '') and date_format(create_time, "%Y-%m-%d") < CURDATE() and date_format(create_time, "%Y-%m-%d") >= "2021-11-28"
            ...
            sell_sql = '''
                ...
                from lh_pretty_client.le_order
                where del_flag=0 and type = 1 and `status`=1 and (sell_phone is not null or sell_phone != '') and date_format(create_time, "%Y-%m-%d") < CURDATE() and date_format(create_time, "%Y-%m-%d") >= "2021-11-28"
            ...
            buy_sql = '''
                select date_format(create_time, "%Y-%m-%d") day_time, phone, count(*) buy_count, sum(count) buy_pretty_count, sum(total_price) buy_total_price
                from lh_pretty_client.le_order
                where del_flag=0 and type = 1 and `status`=1 and (phone is not null or phone != '') and date_format(create_time, "%Y-%m-%d") = date_sub(curdate(), interval 1 day)
                group by day_time, phone
            '''
            # 出售收据
            sell_sql = '''
                ...
                sum(total_price) - sum(sell_fee) truth_price, sum(sell_fee) sell_fee
                from lh_pretty_client.le_order
                where del_flag=0 and type = 1 and `status`=1 and (sell_phone is not null or sell_phone != '') and date_format(create_time, "%Y-%m-%d") = date_sub(curdate(), interval 1 day)
                group by day_time, sell_phone
            '''
            # 发布数据
            publish_sql = '''
        ...
        logger.info('插入数据')
        fina_df.to_sql('user_daily_order_p8', con=conn_an, if_exists="append", index=False)
        return True, '插入成功'
