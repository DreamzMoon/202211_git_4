...
def tran_hold():
    try:
        ...
        select if(hold_phone,hold_phone,hold_user_id) hold_phone, pretty_type_id, date_format(thaw_time,"%Y-%m-%d %H:%i:%S") thaw_time, is_sell, pay_type, date_format(create_time,"%Y-%m-%d %H:%i:%S") create_time, status from lh_pretty_hold_f where del_flag=0 and `status` in (0,1,2) and is_open_vip = 0 and (thaw_time<=now() or create_time<=now())
                '''
