...
def lh_user_add():
    try:
        ...
        param = [crm_data["unionid"],crm_data["parentid"],lh_user_id,lh_status,phone,addtime,updtime,crm_data["sex"],
                 ...
                 crm_data["serpro_grade"],crm_data["serpro_status"],crm_data["serpro_starttime"],0]
        ...
        with conn_rw.cursor() as cursor:
            insert_sql = '''insert into lh_user values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'''
            cursor.execute(insert_sql,param)
            conn_rw.commit()
        conn_rw.close()
        return {"code":0,"status":"success","msg":"数据同步成功"}
