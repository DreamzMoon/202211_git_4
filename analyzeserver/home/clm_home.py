...
def person_top():
    try:
        ...
        #     (select unionid, sum(pay_money) total_money from luke_marketing.orders
...
def shop_top():
    try:
        ...
        #     (select shop_id, sum(pay_money) total_money from luke_marketing.orders
        #     where is_del=0 and pay_status=2
        #     and from_unixtime(addtime, "%Y-%m-%d")=current_date
        #     group by shop_id
        #     order by total_money desc
...
def area_list():
    try:
        ...
        #     orders.unionid, orders.number total_count, orders.pay_money total_money
        ...
        #     where luke_marketing.orders.is_del = 0 and luke_marketing.orders.pay_status=2 and FROM_UNIXTIME(luke_marketing.orders.addtime,'%Y-%m-%d')=current_date
        # '''
        '''近1个月'''
        # area_list_sql = '''
        #     select address_province.PROVINCE_NAME province_name, address_city.CITY_NAME city_name, address_area.AREA_NAME region_name,
        ...
        #     left join luke_marketing.address_city on address_city.CITY_CODE = address_area.CITY_CODE
        #     left join luke_marketing.address_province on address_province.PROVINCE_CODE = address_city.PROVINCE_CODE
        #     where luke_marketing.orders.is_del = 0 and luke_marketing.orders.pay_status=2 and FROM_UNIXTIME(luke_marketing.orders.addtime,'%Y-%m-%d')>=date_sub(curdate(), interval 1 month)
        # '''
        ...
        '''重复统计人数'''
        area_list_sql = '''
            select FROM_UNIXTIME(luke_marketing.orders.addtime,'%Y-%m-%d') day_time, address_province.PROVINCE_NAME province_name, address_city.CITY_NAME city_name, address_area.AREA_NAME region_name,
            orders.unionid, orders.number total_count, orders.pay_money total_money
            from luke_marketing.orders
            left join luke_marketing.shop on orders.shop_id = shop.id
            left join luke_marketing.address_area on address_area.AREA_CODE = shop.area
        ...
        if not province_code and not city_code:
            ...
            if not city_code:
                ...
                # city_name_list = ["'%s'" % city for city in city_list_df['name'].tolist()]
                city_name_list = city_list_df['name'].tolist()
                area_list_df = area_list_df[
                    (area_list_df['province_name'] == province_name) & (area_list_df['city_name'].isin(city_name_list))]
                logger.info(area_list_df.shape)
                area_list_df.drop(['province_name', 'region_name'], axis=1, inplace=True)
                area_list_df.rename(columns={"city_name": "area_name"}, inplace=True)
            else:
                ...
                area_list_df.rename(columns={"region_name": "area_name"}, inplace=True)
        # 人数统计
        area_person_count_df = pd.DataFrame(
            area_list_df.groupby(['area_name', 'day_time'])['unionid'].nunique()).reset_index().rename(
            columns={"unionid": "person_count"})
        area_person_count_df = area_person_count_df.groupby('area_name')['person_count'].sum().reset_index()
        area_list_df = area_list_df.groupby('area_name').agg(
            {"total_count": "sum", "total_money": "sum", "area_name": "count"}).rename(
            columns={"area_name": "order_count"}).reset_index()
        area_list_df = area_list_df.merge(area_person_count_df, how='left', on='area_name')
        # 占比（订单数）
        area_list_df['proportion'] = area_list_df['order_count'] / area_list_df['order_count'].sum()
        area_list_df['proportion'] = area_list_df['proportion'].round(2)
