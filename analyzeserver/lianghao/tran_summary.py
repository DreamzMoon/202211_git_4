...
def user_summary():
    try:
        ...
        # 发布数据
        publish_sql = '''
            select sell_phone phone, count publish_count, total_price publish_total_price, if(up_time is not null,up_time,create_time) up_time from lh_sell where del_flag=0 and status !=1 and sell_phone is not null and sell_phone != ''
        '''
        # 查询官方
        inside_recovery_phone_sql = '''
            select inside_publish_phone from lh_analyze.data_board_settings where del_flag=0 and market_type=1
        '''
        # 靓号用户数据
        lh_user_sql = '''
...
def plat_summary():
    try:
        ...
        buy_sql = '''
            ...
            from lh_order where del_flag=0 and status=1 and type in (1,4) and phone is not null and phone != ''
        '''
        # 发布数据
        publish_sql = '''
            ...
            from lh_sell where del_flag=0 and status != 1 and sell_phone is not null and sell_phone != ''
        '''
        # 查询官方
        inside_recovery_phone_sql = '''
...
def operate():
    try:
        ...
        officical_data.rename(columns={"phone": "phone", "total_price": "official_total_price", "count": "official_count"}, inplace=True)
