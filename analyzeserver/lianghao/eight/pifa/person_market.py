...
# 个人转卖市场发布数据分析
@personpfbp.route('/publish', methods=["POST"])
def personal_publish():
    try:
        ...
        data_sql = '''
            select sell_phone publish_phone, total_price, create_time from lh_pretty_client.le_sell where del_flag = 0 and `status` != 1 and (sell_phone is not null and sell_phone != '')
        '''
...
# 个人转卖市场发布数据分析详情
@personpfbp.route('/publish/detail', methods=["POST"])
def personal_publish_detail():
    try:
        ...
        publish_sql = '''select id sell_id, count, total_price, pretty_type_name pretty_type, create_time from lh_pretty_client.le_sell where del_flag=0 and sell_phone=%s and `status`!=1''' % phone
        user_publish_base_df = pd.read_sql(publish_sql, conn_lh)

        order_sql = '''select sell_id, sell_phone publish_phone, pay_type from lh_pretty_client.le_order where del_flag=0 and sell_phone= %s and `status`=1 and type=1 and pay_type is not null and sell_id is not null''' % phone
...
# 个人转卖市场订单流水
@personpfbp.route('/orderflow', methods=["POST"])
def personal_order_flow():
    try:
        ...
        order_flow_sql = '''
            ...
            left join
            (select id, type transfer_type from lh_pretty_client.le_sell) t2
            on t1.sell_id = t2.id
...
# 个人转卖市场发布出售订单流水
@personpfbp.route('/publishflow', methods=["POST"])
