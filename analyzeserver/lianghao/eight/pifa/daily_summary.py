...
daily_eight_pifabp = Blueprint('daily_eight_pifa', __name__, url_prefix='/le/pifa/daily')
...
'''平台每日订单数据统计报表'''
@daily_eight_pifabp.route("plat",methods=["POST"])
def daily_plat_summary():
    try:
        ...
        buy_sql = '''select DATE_FORMAT(create_time,"%Y-%m-%d") statistic_time,count(*) buy_order_count,sum(count) buy_lh_count,sum(total_price) buy_total_price from le_order where type = 1 and `status`=1 and del_flag = 0'''
        sell_sql = '''select DATE_FORMAT(create_time,"%Y-%m-%d") statistic_time,count(*) sell_order_count,sum(count) sell_lh_count,sum(total_price) sell_total_price,sum(sell_fee) total_sell_fee,sum(total_price)-sum(sell_fee) total_real_money  from le_order where type = 1 and `status`=1 and del_flag = 0 '''
        public_sql = '''select DATE_FORMAT(create_time,"%Y-%m-%d") statistic_time,count(*) public_order_count,sum(count) public_lh_count,sum(total_price) public_total_price from le_sell where del_flag = 0  and `status` != 1'''
...
# 用户每日订单数据统计报表
@daily_eight_pifabp.route("user",methods=["POST"])
def daily_user_summary():
    try:
        ...
        base_sql = '''select day_time,if(`name` is not null and `name`!='',`name`,if(nickname is not null,nickname,"")) nickname, phone, unionid, operate_id, operatename, parentid, parent_phone, buy_count, buy_pretty_count,
                    ...
                    from lh_analyze.user_daily_order_p8'''
...
'''运营中心每日订单数据统计报表'''
@daily_eight_pifabp.route("operate",methods=["POST"])
def daily_operate():
    try:
        conn_analyze = direct_get_conn(analyze_mysql_conf)
        logger.info(request.json)
        ...
        sql = '''
        select day_time,operate_id,operatename,leader,leader_phone,leader_unionid ,sum(buy_count) buy_count,sum(buy_pretty_count) buy_pretty_count,sum(buy_total_price) buy_total_price,sum(publish_count) publish_count,sum(publish_pretty_count) publish_pretty_count,sum(publish_total_price) publish_total_price,sum(sell_count) sell_count,sum(sell_pretty_count) sell_pretty_count,sum(sell_total_price) sell_total_price,sum(truth_price) truth_price,sum(sell_fee)sell_fee from user_daily_order_p8 where operatename != "" group by day_time,operatename order by day_time desc,buy_total_price desc
        '''
...
'''平台每日名片网估值'''
@daily_eight_pifabp.route("plat/value",methods=["POST"])
def daily_plat_value():
    try:
        ...
        sql = '''select day_time,sum(no_tran_price) no_tran_price,sum(no_tran_count) no_tran_count,sum(transferred_count) transferred_count,sum(transferred_price) transferred_price,sum(public_count_pifa) public_count,sum(public_price_pifa) public_price,sum(use_total_price) use_total_price,sum(use_count) use_count,sum(hold_price) hold_price,sum(hold_count) hold_count,sum(tran_price) tran_price,sum(tran_count) tran_count from user_storage_eight'''
...
@daily_eight_pifabp.route("operate/value",methods=["POST"])
def daily_operate_value():
    try:
        ...
        sql = '''
        select day_time,operate_id,operatename,leader,leader_phone,leader_unionid ,sum(no_tran_price) no_tran_price,sum(no_tran_count) no_tran_count,sum(transferred_count) transferred_count,sum(transferred_price) transferred_price,sum(public_count_pifa) public_count,sum(public_price_pifa) public_price,sum(use_total_price) use_total_price,sum(use_count) use_count,sum(hold_price) hold_price,sum(hold_count) hold_count,sum(tran_price) tran_price,sum(tran_count) tran_count from user_storage_eight where operatename != "" group by day_time,operatename order by day_time desc,hold_count desc'''
...
@daily_eight_pifabp.route("user/value",methods=["POST"])
def daily_ser_value():
    try:
        conn_an = direct_get_conn(analyze_mysql_conf)
        logger.info(request.json)
        ...
        sql = '''select day_time, if(`name` is not null and `name`!='',`name`,if(nickname is not null,nickname,"")) nickname, hold_phone, unionid, operate_id, operatename, parentid, parent_phone, no_tran_price, no_tran_count,
                    transferred_count, transferred_price, public_count_pifa public_count,public_price_pifa public_price, use_total_price, 
