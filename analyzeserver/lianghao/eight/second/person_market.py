...
# 个人转卖市场发布数据分析
@personsecondbp.route('/publish', methods=["POST"])
...
# 个人转卖市场发布数据分析详情
@personsecondbp.route('/publish/detail', methods=["POST"])
...
# 个人转卖市场订单流水
@personsecondbp.route('/orderflow', methods=["POST"])
...
# 个人转卖市场发布出售订单流水
@personsecondbp.route('/publishflow', methods=["POST"])
def personal_publish_order_flow():
    try:
        ...
        logger.info(match_result[2].shape[0])
        return {"code": "0000", "status": "success", "msg": return_data, "count": match_result[2].shape[0]}
    except Exception as e:
        logger.error(traceback.format_exc())
        return {"code": "10000", "status": "failed", "msg": message["10000"]}
...
'''个人转卖市场采购数据分析总'''
@personsecondbp.route("buy/all",methods=["POST"])
def personal_buy_all():
    try:
        ...
        buy_sql = '''select phone,total_price,create_time from le_order where `status` = 1 and  del_flag = 0 and type = 4'''
...
'''个人转卖市场采购数据分析--个人'''
@personsecondbp.route("buy",methods=["POST"])
def person_buy():
    try:
        conn_read = direct_get_conn(lianghao_mysql_conf)
        logger.info(request.json)
        ...
        all_sql = '''select count(*) order_count,sum(count) total_count,sum(total_price) total_price,GROUP_CONCAT(pay_type) sum_pay_type from le_order where del_flag = 0 and type = 4 and `status`=1 group by phone having phone = %s'''
        ...
        if time_type == 1 or (time_type == 4 and daysss and daysss.days + daysss.seconds / (24.0 * 60.0 * 60.0)<1):
            ...
            select DATE_FORMAT(create_time, '%%Y-%%m-%%d') statistic_time,sum(total_price) buy_total_price,count(*) buy_order_count from le_order where `status` = 1 and  del_flag = 0 and type = 4 and phone = %s and DATE_FORMAT(create_time, '%%Y-%%m-%%d')<=DATE_ADD(CURRENT_DATE(),INTERVAL %s day) and DATE_FORMAT(create_time, '%%Y-%%m-%%d')>=DATE_ADD(CURRENT_DATE(),INTERVAL %s day) group by statistic_time order by statistic_time asc) a
            union all
            ...
            # 查询近七天的
            today_sql = '''select DATE_FORMAT(create_time, '%%Y-%%m-%%d') statistic_time,count(*) buy_order_count,sum(count) buy_total_count,sum(total_price) buy_total_price from le_order where `status` = 1 and  del_flag = 0 and type = 4 and phone = %s and DATE_FORMAT(create_time, '%%Y-%%m-%%d')<=DATE_ADD(CURRENT_DATE(),INTERVAL %s day) and DATE_FORMAT(create_time, '%%Y-%%m-%%d')>=DATE_ADD(CURRENT_DATE(),INTERVAL %s day) group by statistic_time order by statistic_time asc ''' %(phone,query_range[0],query_range[1])
            ...
            circle_sql = '''
                        select "current" week,if(sum(buy_total_price),sum(buy_total_price),0) buy_total_price,if(sum(buy_order_count),sum(buy_order_count),0) buy_order_count from(
                        select DATE_FORMAT(create_time, '%%Y-%%m-%%d') statistic_time,sum(total_price) buy_total_price,count(*) buy_order_count from le_order where `status` = 1 and  del_flag = 0 and type = 4 and phone = %s and create_time<="%s" and create_time>="%s" group by statistic_time order by statistic_time asc) a
                        union all
                        select "last" week,if(sum(buy_total_price),sum(buy_total_price),0) buy_total_price,if(sum(buy_order_count),sum(buy_order_count),0) buy_order_count from(
                        select DATE_FORMAT(create_time, '%%Y-%%m-%%d') statistic_time,sum(total_price) buy_total_price,count(*) buy_order_count from le_order where `status` = 1 and  del_flag = 0 and type = 4 and phone = %s and create_time<="%s" and create_time>="%s" group by statistic_time order by statistic_time asc) b ''' %(phone,end_time,start_time,phone,before_end_time,before_start_time)
            ...
            today_sql = '''select DATE_FORMAT(create_time, '%%Y-%%m-%%d') statistic_time,count(*) buy_order_count,sum(count) buy_total_count,sum(total_price) buy_total_price from le_order where `status` = 1 and  del_flag = 0 and type = 4 and phone = %s and create_time <= "%s" and create_time >= "%s" group by statistic_time order by statistic_time asc ''' %(phone,end_time,start_time)
...
'''个人转卖市场出售数据分析总'''
@personsecondbp.route("sell/all", methods=["POST"])
def personal_sell_all():
    try:
        ...
        buy_sql = '''select sell_phone phone,total_price,create_time from le_order where `status` = 1 and  del_flag = 0 and type = 4'''
        if time_condition_sql:
            buy_sql = buy_sql + time_condition_sql
...
'''个人转卖市场出售数据分析--个人'''
@personsecondbp.route("sell", methods=["POST"])
def person_sell():
    try:
        logger.info(request.json)
        conn_read = direct_get_conn(lianghao_mysql_conf)
        ...
        if time_type == 1 or (time_type == 4 and daysss and daysss.days + daysss.seconds / (24.0 * 60.0 * 60.0)<1):
            ...
            circle_sql = '''
            select "current" week,if(sum(buy_total_price),sum(buy_total_price),0) buy_total_price,if(sum(buy_order_count),sum(buy_order_count),0) buy_order_count from(
            select DATE_FORMAT(create_time, '%%Y-%%m-%%d') statistic_time,sum(total_price) buy_total_price,count(*) buy_order_count from le_order where `status` = 1 and  del_flag = 0 and type = 4 and sell_phone = %s and DATE_FORMAT(create_time, '%%Y-%%m-%%d')<=DATE_ADD(CURRENT_DATE(),INTERVAL %s day) and DATE_FORMAT(create_time, '%%Y-%%m-%%d')>=DATE_ADD(CURRENT_DATE(),INTERVAL %s day) group by statistic_time order by statistic_time asc) a
            union all
            select "last" week,if(sum(buy_total_price),sum(buy_total_price),0) buy_total_price,if(sum(buy_order_count),sum(buy_order_count),0) buy_order_count from(
            ...
            # 查询近七天的
            today_sql = '''select DATE_FORMAT(create_time, '%%Y-%%m-%%d') statistic_time,count(*) buy_order_count,sum(count) buy_total_count,sum(total_price) buy_total_price from le_order where `status` = 1 and  del_flag = 0 and type = 4 and sell_phone = %s and DATE_FORMAT(create_time, '%%Y-%%m-%%d')<=DATE_ADD(CURRENT_DATE(),INTERVAL %s day) and DATE_FORMAT(create_time, '%%Y-%%m-%%d')>=DATE_ADD(CURRENT_DATE(),INTERVAL %s day) group by statistic_time order by statistic_time asc ''' % (
            ...
            circle_sql = '''
                        select "current" week,if(sum(buy_total_price),sum(buy_total_price),0) buy_total_price,if(sum(buy_order_count),sum(buy_order_count),0) buy_order_count from(
                        select DATE_FORMAT(create_time, '%%Y-%%m-%%d') statistic_time,sum(total_price) buy_total_price,count(*) buy_order_count from le_order where `status` = 1 and  del_flag = 0 and type = 4 and sell_phone = %s and create_time<="%s" and create_time>="%s" group by statistic_time order by statistic_time asc) a
                        union all
                        select "last" week,if(sum(buy_total_price),sum(buy_total_price),0) buy_total_price,if(sum(buy_order_count),sum(buy_order_count),0) buy_order_count from(
                        select DATE_FORMAT(create_time, '%%Y-%%m-%%d') statistic_time,sum(total_price) buy_total_price,count(*) buy_order_count from le_order where `status` = 1 and  del_flag = 0 and type = 4 and sell_phone = %s and create_time<="%s" and create_time>="%s" group by statistic_time order by statistic_time asc) b ''' % (
            ...
            today_sql = '''select DATE_FORMAT(create_time, '%%Y-%%m-%%d') statistic_time,count(*) buy_order_count,sum(count) buy_total_count,sum(total_price) buy_total_price from le_order where `status` = 1 and  del_flag = 0 and type = 4 and sell_phone = %s and create_time <= "%s" and create_time >= "%s" group by statistic_time order by statistic_time asc ''' % (
