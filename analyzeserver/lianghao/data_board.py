...
def lh_personboard_sell():
    try:
        ...
        # 上架总数
        sell_sum_count_sql = '''select sum(count) sell_count from lh_sell where `status` = 0 and del_flag = 0'''

        # 上架价值
        sell_sum_price_sql = '''select sum(total_price) sell_total_price from lh_sell where `status` = 0 and del_flag = 0'''

        # 内部渠道上架总数
        inside_sell_count_sql = '''select sum(count) inside_sell_count from lh_sell where `status` = 0 and del_flag = 0 and sell_phone in (%s)''' % (
        kanban_data[0]["inside_publish_phone"][1:-1])
        # 内部渠道商家价值
        inside_sell_price_sql = '''select sum(total_price) inside_sell_total_price from lh_sell where `status` = 0 and del_flag = 0 and sell_phone in (%s)''' % (
        kanban_data[0]["inside_publish_phone"][1:-1])
        ...
        # 最早上架时间
        early_sql = '''select up_time from lh_sell where `status` = 0 and del_flag = 0 and sell_phone not in (%s)''' % (kanban_data[0]["inside_publish_phone"][1:-1])
