...
def personal_hold_total():
    try:
        ...
        today_storage_sql = '''
            select a.hold_phone, a.transferred_count, a.transferred_price, a.no_tran_count, a.no_tran_price, a.hold_count, a.hold_price, a.tran_count, a.tran_price from lh_analyze.user_storage_value_today a
            join (select hold_phone, max(addtime) time from lh_analyze.user_storage_value_today group by hold_phone) b
            on a.hold_phone = b.hold_phone
            and a.addtime = b.time
        '''
        ...
        if user_condition != '':
            ...
            user_storage_value_sql = user_storage_value_sql.format(''' where hold_phone in (%s)''' % ','.join(condition_phone_list))
